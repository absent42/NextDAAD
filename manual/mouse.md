...
| 3 | `GETMS` | Read mouse state into four flags starting at the first argument: `flags[n]` = buttons (idle 0, left 1, right 2, middle 4, chords additive - jdaad parity, not the raw Kempston byte), `flags[n+1]` = column 0-79 (X/8), `flags[n+2]` = row 0-31 (Y/8), `flags[n+3]` = column 0-53 (X/6). In 40-column text mode (`GFX 1 18`, see [Graphics](graphics.md#gfx-sub-commands)) `flags[n+1]` clamps to 0-39 instead, so it always spans the full text grid at whichever width is current. |
