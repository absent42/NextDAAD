...
def extract_source(ffmpeg, mp4, width, height, fps, retime, start, duration):
    ...
    (read-only imports - parity by reuse, not reimplementation).

    start/duration are seconds as plain Python floats; stringify them
    HERE, not in videnc.py - videnc.extract_video's own CLI only ever
    sees argparse strings for these and does not stringify internally,
    so a raw float `start` reaching subprocess.run() there raises
    TypeError. This is the seam between vidtune and the pinned videnc
    module."""
    ffmpeg = Path(ffmpeg)
    mp4 = Path(mp4)
