...
def run_payload(buf, pos, surface, cursor_len, palette_out=None, issues=None,
                start_cursor=0, geom=None, covered=None):
    ...
    cursor back in here.
      ...
      geom            (width, height, gapped) - accepted and ignored
                      by the live op set; kept so span callers need no
                      signature special-casing.
      covered         optional bool ndarray(cursor_len) - every byte
                      WRITTEN (RUN/COPY) is marked, for the span
