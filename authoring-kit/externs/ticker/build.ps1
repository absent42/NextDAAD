# build.ps1 - assembles ticker.asm into GAME.XBN in this directory.
...
try {
    & $sjasmplus.Source --msg=war -I "$kitRoot" ticker.asm
    if ($LASTEXITCODE -ne 0) { throw "ticker.asm assembly failed" }
}
finally {
    Pop-Location
}
