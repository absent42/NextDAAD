# build.ps1 - assembles transcript.asm into GAME.XBN in this directory.
...
try {
    & $SjasmPlus --msg=war -I "$kitRoot" transcript.asm
    if ($LASTEXITCODE -ne 0) { throw "transcript.asm assembly failed" }
}
finally {
    Pop-Location
}
