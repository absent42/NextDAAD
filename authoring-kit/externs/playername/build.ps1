# build.ps1 - assembles playername.asm into GAME.XBN in this directory.
...
try {
    & $SjasmPlus --msg=war -I "$kitRoot" playername.asm
    if ($LASTEXITCODE -ne 0) { throw "playername.asm assembly failed" }
}
finally {
    Pop-Location
}
