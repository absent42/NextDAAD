# build.ps1 - assembles clock.asm into GAME.XBN in this directory.
...
try {
    & $SjasmPlus --msg=war -I "$kitRoot" clock.asm
    if ($LASTEXITCODE -ne 0) { throw "clock.asm assembly failed" }
}
finally {
    Pop-Location
}
