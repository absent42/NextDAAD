# build.ps1 - assembles realtime.asm into GAME.XBN in this directory.
...
try {
    & $SjasmPlus --msg=war -I "$kitRoot" realtime.asm
    if ($LASTEXITCODE -ne 0) { throw "realtime.asm assembly failed" }
}
finally {
    Pop-Location
}
