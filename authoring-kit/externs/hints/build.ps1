# build.ps1 - assembles hints.asm into GAME.XBN in this directory.
...
try {
    & $SjasmPlus --msg=war -I "$kitRoot" hints.asm
    if ($LASTEXITCODE -ne 0) { throw "hints.asm assembly failed" }
}
finally {
    Pop-Location
}
