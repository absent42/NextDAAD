...
#              003.VID <- 001_256x240autopal[c10g10s10].vid (fmt 2, 129 sect/frame) [KNOWN BAD, see below]
#              004.VID <- 001_256x240auto[c10g10s10].vid     (fmt 3, 128 sect/frame)
#              005.VID <- GENERATED by tests\gen_fmt4.py from 001_256x192auto
...
#              003.VID size 33156096  64758 sect  -> 2 (129)  [classifies OK, CONTENT malformed]
...
#            third, proof positive). 001/003's (320x240/256x240 autopal)
#            carry the identical stride-3 RGB signature. Three independent
...
#            pending. 001/003 stay staged AS-IS (classify correctly, so
#            they still exercise the classifier and formats 0/2 are
#            future tasks) but are KNOWN BAD content - do not use them to
#            judge formats 0/2 playback until MakeVid is fixed or T4
#            generates synthetics the same way this task does for 005.
#            005.VID sidesteps the SAME defect for THIS task's format-4
#            leg: tests\gen_fmt4.py (new, SP13 T2 closing wave) reads the
#            KNOWN-GOOD 006.VID source (001_256x192auto - never MakeVid's
#            broken autopal output) and re-packs each frame with a
#            synthetic identity 9-bit palette inserted - a genuinely
#            spec-compliant fmt4 file whose audio/pixel content is
#            IDENTICAL to 006.VID's. 006.VID itself needs no such
#            treatment (SP13 Task 2's own STAGED-COPY truncation, below,
#            is all it needs - it was never touched by MakeVid's palette
#            pipeline in the first place, being the no-palette format).
#            005/006's SOURCE fixtures both carry 2 trailing sectors
#            beyond a whole number of frames (98x751+2, consistent
#            MakeVid output for the 256x192 no-palette path - seen in
#            both the pre-incident and regenerated encodes; playvid's own
#            size-only classifier has the same limitation, its README
#            offers -4/-5 to force the format). 006.VID's STAGED COPY
#            (never the tools\ source - read-only) is TRUNCATED to its
#            last whole-frame boundary (fmt5 frame = 98*512=50176 B)
#            immediately after copying, dropping only the 2-sector tail
#            padding, never a whole frame; gen_fmt4.py performs the
#            equivalent truncation internally against the SAME source
#            before generating 005.VID, so both classify clean and the T2
#            player's owner leg (GFX n 13/14, SFX n 9/10) reaches the run
#            loop for both formats 4 and 5.
...
if ($Vid) {
    ...
    # 005.VID is NOT copied here (see below) - MakeVid's autopal encoder
    # is malformed (raw RGB24, never indexed - the -Vid header comment
    # above has the full diagnosis) and tests\gen_fmt4.py generates a
    # spec-compliant fixture in its place instead.
    ...
    # SP13 Task 2 closing wave: 005.VID is GENERATED (not copied) from the
    # SAME known-good 006.VID source, re-packed with a synthetic identity
    # 9-bit palette per frame - see tests\gen_fmt4.py's own header for the
    # full rationale (MakeVid's autopal output is malformed) and the exact
    ...
    if (Test-Path -LiteralPath $fmt4Src) {
        & python "$root\tests\gen_fmt4.py" $fmt4Src "$root\sd\005.VID"
        if ($LASTEXITCODE -ne 0) { throw "gen_fmt4.py failed (exit $LASTEXITCODE) - sd\005.VID not staged" }
        $vidStaged++
    }
    else {
        "WARNING: $fmt4Src missing - sd\005.VID not generated"
    }
    "staged $vidStaged video fixture(s) -> sd\001.VID..sd\006.VID (tools\demo-files MakeVid matrix, classification-priority order; 005.VID synthetic)"
}
