...
#              001.VID <- 001_320x240autopal[c10g10s10].vid (fmt 0, 155 sect/frame) [KNOWN BAD, see below]
...
#              001.VID size 39838720  77810 sect  -> 0 (155)  [classifies OK, CONTENT malformed]
...
#            third, proof positive). 001's (320x240 autopal) carries the
...
#            pending. 001.VID stays staged AS-IS (classifies correctly, so
#            it still exercises the classifier and format 0 is a future
#            task) but is KNOWN BAD content - do not use it to judge
#            format 0 playback until MakeVid is fixed or a future task
#            generates a synthetic the same way this task does for 003/005.
#            003.VID and 005.VID sidestep the SAME defect for the palette
#            formats' (2 and 4) player legs: tests\gen_vid_synth.py (SP13
#            T2 closing wave, generalized in T3) reads the KNOWN-GOOD
...
if ($Vid) {
    ...
    "staged $vidStaged video fixture(s) -> sd\001.VID..sd\006.VID (tools\demo-files MakeVid matrix, classification-priority order; 003.VID/005.VID synthetic)"
}
