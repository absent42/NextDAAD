...
#              001.VID <- GENERATED by tests\videnc.py from tools\demo-
#                         files\001.mp4 (fmt 0, 155 sect/frame) - SP13 T4,
#                         see below (MakeVid's own autopal encode for this
#                         format is malformed, same defect as 003/005)
#              002.VID <- 001_320x240auto[c10g10s10].vid     (fmt 1, 154 sect/frame)
#              003.VID <- GENERATED by tests\gen_vid_synth.py from 001_256x240auto
#                         [c10g10s10].vid (fmt 2, 129 sect/frame) - see below
#              004.VID <- 001_256x240auto[c10g10s10].vid     (fmt 3, 128 sect/frame)
#              005.VID <- GENERATED by tests\gen_vid_synth.py from 001_256x192auto
#                         [c10g10s10].vid (fmt 4, 99 sect/frame) - see below
...
#              001.VID size 39838720  77810 sect  -> 0 (155)  [REAL adaptive-palette encode, SP13 T4]
#              002.VID size 39581696  77308 sect  -> 1 (154)  [intended 1, correct]
#              003.VID size 33156096  64758 sect  -> 2 (129)  [SYNTHETIC, spec-compliant]
#              004.VID size 32899072  64256 sect  -> 3 (128)  [intended 3, correct]
#              005.VID size 38066688  74349 sect  -> 4 (99)   [SYNTHETIC, spec-compliant]
#              006.VID size 37682176  73598 sect  -> 5 (98)   [intended 5, correct]
#            MAKEVID 1.77 "autopal" ENCODES ARE MALFORMED (owner + SP13 T2
#            closing-wave finding): they carry raw RGB24 pixel data, never
#            converted to indexed colour (256x192 RGB24 = 147456 B vs the
#            format's 49152-byte pixel slot - decoding just the top third
#            as 256x64x3 RGB yields a pristine PNG of the test card's top
#            third, proof positive). 001's (320x240 autopal) carried the
#            identical stride-3 RGB signature. Three independent
#            renderers (this project's player, playvid itself, and an
#            offline decode) garble/misread these files identically -
#            MakeVid's IrfanView-dependent custom-palette pipeline
#            silently falling back to raw frames is the suspected
#            mechanism (its own README: custom palette "requires an
#            external tool call IrfanView install"); upstream report
#            pending. SP13 T4 replaces 001.VID's staging with a REAL
#            encode via tests\videnc.py --format 0 (the project's own
#            open encoder, proven for fmt2/fmt4 - see the encode block
#            below) from tools\demo-files\001.mp4 - the first REAL per-
#            frame-adaptive-palette content through the player (003.VID/
#            005.VID below are synthetic identity palettes, not adaptive
#            quantization). 003.VID and 005.VID sidestep the SAME MakeVid
#            defect for the palette formats' (2 and 4) player legs via a
#            DIFFERENT technique: tests\gen_vid_synth.py (SP13 T2 closing
#            wave, generalized in T3) reads the KNOWN-GOOD
#            004.VID/006.VID sources (001_256x240auto/001_256x192auto -
#            never MakeVid's broken autopal output) and re-packs each
#            frame with a synthetic identity 9-bit palette inserted -
#            genuinely spec-compliant fmt2/fmt4 files whose audio/pixel
#            content is IDENTICAL to their no-palette siblings'. 004.VID/
#            006.VID themselves need no such treatment (their own STAGED-
#            COPY handling, below, is all they need - neither was ever
#            touched by MakeVid's palette pipeline in the first place,
#            being the no-palette formats). 005/006's SOURCE fixtures both
#            carry 2 trailing sectors beyond a whole number of frames
#            (98x751+2, consistent MakeVid output for the 256x192 no-
#            palette path - seen in both the pre-incident and regenerated
#            encodes; playvid's own size-only classifier has the same
#            limitation, its README offers -4/-5 to force the format).
#            004's source (256x240 no-palette) has no such remainder -
#            32899072/65536=502.0 exactly. 006.VID's STAGED COPY (never
#            the tools\ source - read-only) is TRUNCATED to its last
#            whole-frame boundary (fmt5 frame = 98*512=50176 B)
#            immediately after copying, dropping only the 2-sector tail
#            padding, never a whole frame; gen_vid_synth.py performs the
#            equivalent truncation internally against the SAME source
#            before generating 005.VID (a no-op for 003.VID's own 004.VID-
#            sourced generation, already frame-aligned), so all four
#            (003/004/005/006) classify clean and the player's owner leg
#            (GFX n 13/14, SFX n 9/10) reaches the run loop for formats
#            2-5. 001.VID/002.VID (SP13 T4, formats 0/1) reach the SAME
#            run loop - see the videnc.py encode block below for 001's
#            own generation/caching.
...
if ($Vid) {
    ...
    # 003.VID and 005.VID are NOT copied here (see below) - MakeVid's
    # autopal encoder is malformed (raw RGB24, never indexed - the -Vid
    # header comment above has the full diagnosis) and tests\
    # gen_vid_synth.py generates spec-compliant fixtures in their place
    # instead (SP13 T2 closing wave for 005; T3 for 003).
    ...
    # SP13 Task 2: truncate the STAGED COPY of 006.VID (256x192 no-palette)
    # to its last whole-frame boundary - the source fixture carries a
    # 2-sector MakeVid tail beyond a whole number of frames (see the -Vid
    # header comment above), which leaves it colliding with an earlier
    # format's divisor. Truncating drops only the tail padding, never a
    # whole frame. tools\demo-files itself is never modified (read-only
    # per project rules) - this rewrites only the sd\ copy staged above.
    $vidFrameBytes = [ordered]@{ '006.VID' = 50176 }   # fmt5 98*512
    ...
    # SP13 T2 closing wave / T3: 005.VID and 003.VID are GENERATED (not
    # copied) from their matching known-good no-palette siblings (006/004
    # respectively), re-packed with a synthetic identity 9-bit palette per
    # frame - see tests\gen_vid_synth.py's own header for the full
    # rationale (MakeVid's autopal output is malformed) and the exact
    # palette derivation. The script performs its own whole-frame
    # truncation against the untouched source, independent of the 006.VID
    # copy/truncate above (004.VID's own source is already frame-aligned -
    # 32899072 / 65536 = 502.0 exactly - so no truncation message prints
    # for the 003.VID generation pass, unlike 005's).
    $fmt4Src = Join-Path $vidSrc '001_256x192auto[c10g10s10].vid'
    if (Test-Path -LiteralPath $fmt4Src) {
        & python "$root\tests\gen_vid_synth.py" 4 $fmt4Src "$root\sd\005.VID"
        if ($LASTEXITCODE -ne 0) { throw "gen_vid_synth.py failed (exit $LASTEXITCODE) - sd\005.VID not staged" }
        $vidStaged++
    }
    else {
        "WARNING: $fmt4Src missing - sd\005.VID not generated"
    }
    $fmt2Src = Join-Path $vidSrc '001_256x240auto[c10g10s10].vid'
    if (Test-Path -LiteralPath $fmt2Src) {
        & python "$root\tests\gen_vid_synth.py" 2 $fmt2Src "$root\sd\003.VID"
        if ($LASTEXITCODE -ne 0) { throw "gen_vid_synth.py failed (exit $LASTEXITCODE) - sd\003.VID not staged" }
        $vidStaged++
    }
    else {
        "WARNING: $fmt2Src missing - sd\003.VID not generated"
    }
    # SP13 T4: 001.VID (fmt 0, 320x240 palette) is a REAL adaptive-palette
    # encode via tests\videnc.py (the project's own open encoder, proven
    # for fmt2/fmt4 - .superpowers\sdd\sp13-task-e-report.md), from
    # tools\demo-files\001.mp4 (the master source the other per-format
    # fixtures were themselves cut from - .superpowers\sdd\sp13-task-4-
    # report.md). This is the FIRST real per-frame-adaptive-palette
    # content through the player (003.VID/005.VID above are synthetic
    # identity palettes, not adaptive quantization) - unlike 003/005,
    # there is no known-good no-palette sibling to re-pack (001's own
    # MakeVid autopal source is the malformed one; 002.VID, its no-
    # palette sibling, is genuinely fine and stays a plain copy above).
    # The encode is SLOW (per-frame adaptive quantization over ~502
    # frames) so the result is CACHED at tests\out\001_fmt0_cache.vid and
    # only regenerated when that cache file is missing - tests\out\ is
    # gitignored (persists across runs, unlike sd\*.VID which this switch
    # always stale-cleans first). Regenerate by deleting the cache file
    # and re-running -Vid, or directly with:
    #   python tests\videnc.py tools\demo-files\001.mp4 tests\out\001_fmt0_cache.vid --format 0
    $fmt0Cache = Join-Path $root 'tests\out\001_fmt0_cache.vid'
    $fmt0Src = Join-Path $vidSrc '001.mp4'
    if (-not (Test-Path -LiteralPath $fmt0Cache)) {
        if (Test-Path -LiteralPath $fmt0Src) {
            "encoding sd\001.VID (fmt 0) via videnc.py - slow, cached at tests\out\001_fmt0_cache.vid after this run..."
            & python "$root\tests\videnc.py" $fmt0Src $fmt0Cache --format 0
            if ($LASTEXITCODE -ne 0) { throw "videnc.py failed (exit $LASTEXITCODE) - sd\001.VID not staged" }
        }
        else {
            "WARNING: $fmt0Src missing - sd\001.VID not generated"
        }
    }
    if (Test-Path -LiteralPath $fmt0Cache) {
        Copy-Item -LiteralPath $fmt0Cache -Destination "$root\sd\001.VID" -Force
        $vidStaged++
    }
    "staged $vidStaged video fixture(s) -> sd\001.VID..sd\006.VID (tools\demo-files MakeVid matrix, classification-priority order; 001.VID/003.VID/005.VID generated, not plain copies)"
}
