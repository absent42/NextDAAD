...
try {
    ...
    # Glyph 65 gets a marker pattern; must appear at both 65*8 and 193*8.
    ...
    # Glyph 193 must survive untouched - the mirror must not touch it.
    ...
    # 112 glyphs of 8 rows from character 16 - the NextZXOS CP/M font shape.
    ...
    # Every row and column set: the tightest input that must still be
    # ACCEPTED. Raw dumps have no header to declare a height, so the
    # refusal side is proved instead by P3/P4 below (PSF2, 16-row cell).
    ...
    #     127 ---
    # This shape self-identifies as the ZX charset, whose 96/127 are
    # already pound/copyright in-face - must not be substituted from the
    # base font. Patterns differ from the base font's so a swap can't pass unnoticed.
    ...
    # Same raw shape as G3, no declared charset ordering - unlike G6,
    ...
    # cga80woa.fon ships with Windows (CP437, 8x8) - nothing to install.
    ...
    if (Test-Path $cga) {
        ...
        # dfCharSet OEM: glyph 96 must be CP437 slot 156, not the base font's.
    ...
    # pack - never the only proof, never a silent skip.
    ...
    #     raw .NET exception ---
    # Resource table offset (0xFFFF) points past the end of the buffer -
    # exercises the bounds guard on $rsrc in Read-FontFon.
    ...
    # One 8x8 face (auto-picked), one character-table entry declaring
    # width 9. F4 = code 65 (refused), F5 = code 200 (dropped).
    ...
    # Inside 32-127 this must be refused, naming the code - a skipped
    # glyph would silently leave the base font's glyph under a clean
    # success line.
    ...
    #     other lifted glyph ---
    # OEM slot 156 has ink on row 8 (9-row face) - must not be lifted
    ...
    # raw dump (UDGs/decorative glyphs) - a dump landing wholly in either
    # range must convert, not just 32-127.
    ...
    # 256 glyphs of 8 rows; glyph 65's marker must mirror to 193.
    ...
    # one must convert. Measured, not declared.
    ...
    #     cropped ---
    # File is otherwise valid (charsize matches 2 bytes/row * 8 rows) -
    # only the width check can fail it.
    ...
    # P6a: 256-glyph-sized file with bit 0 set - honoured, it demands 512
    ...
    # FONT_ASCENT 7: 'A' (BBX yoff 0) lands at rows 0-6, 'g' (BBX yoff
    # -1) lands at rows 1-7.
    $bdf = @'
STARTFONT 2.1
...
'@
    ...
    # Same 12-wide glyph, different code - proves the OverWide route,
...
"@
    ...
    # 'B' has no BBX after 'A's - a parser carrying bbx over between
    # glyphs would silently reuse 'A's; must refuse, naming the glyph.
    $bdfNoBbx = @'
STARTFONT 2.1
FONT -test-nobbx
SIZE 8 75 75
...
'@
    ...
    #     message, not a raw .NET exception ---
    # 14 nines overflows Int32 outright - deterministic on every
    # machine, unlike a huge-but-valid allocation size.
    $bdfBigAscent = @'
STARTFONT 2.1
FONT -test-bigascent
SIZE 8 75 75
...
'@
    ...
    # 8 bytes/glyph, row major, MSB left, no header - identical to a raw
    # glyph table. Pins that so raw-handling changes can't break .spr.
    ...
    # -font-y interleaves rows within each tile row (blk[r*columns+g]);
    # same file length as -font, column count unrecorded, so nothing can
    # tell the two apart except this warning - it must name -font-y.
