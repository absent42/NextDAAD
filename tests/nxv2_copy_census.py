...
def frames_of(vid_path):
    """Yield one list of (opcode, length) per source frame."""
    buf = Path(vid_path).read_bytes()
    hdr = nxv2enc.unpack_header(buf)
    for _pal, _surf, _term, start, length in nxv2dec._iter_frames(buf, hdr, issues=[]):
...
def worst_gapped_fraction(vid_paths, surcharge_t, copy_thr):
    ...
    nxv2enc.op_cost(kind, length) returns (bytes, T)."""
    worst = 0.0
    for p in vid_paths:
        if not is_gapped_file(p):
            continue
        for ops in frames_of(p):
            modelled = nxv2enc.TMODEL_COEFFS["t_frame_fixed"]
            added = 0.0
            for op, n in ops:
