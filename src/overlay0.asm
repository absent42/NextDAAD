...
; EXTERN vector 6 (SP13 Task 1): DEBUG-only route to VIDBENCH (tests/
; test.dsf), the video streaming benchmark harness (video.asm, VID_PAGE).
; Chosen over 5 (suite check 44 runs "EXTERN 5 5" and depends on vector 5
; staying ext_stub forever) and over 3/4/7 (live: XMESSAGE/XPART/
; XUNDONE). extVec is overlay0 DATA, not code - the IFDEF below on the
; table ROW costs nothing in a release build (the row is simply ext_stub
; there, byte-identical to before this task); the trampoline body is
; itself IFDEF DEBUG so no dead code reaches overlay0 in release either.
 IFDEF DEBUG
vid_bench_trampoline:
    ld hl, vid_bench              ; established push-target/ovl_map_page
    push hl                       ; trampoline idiom (font_load_switch,
    ld a, VID_PAGE                ; xpart_load_fail's own hop, etc.)
    jp ovl_map_page
 ENDIF
...
extVec:
    ...
    dw h_xpart, ext_stub
 IFDEF DEBUG
    dw vid_bench_trampoline       ; vector 6, DEBUG only
 ELSE
